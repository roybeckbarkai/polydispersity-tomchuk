...
Across the full benchmark, the average absolute error in recovered $p$ was 0.367 for PDI and 0.189 for PDI2, while the overall sanity-pass rate was 7.9\%.
...
The photon-count panel shows a rapid improvement of the low-$q$ radius term: the mean absolute relative error in $R_g$ falls from 0.294 at $10^{5}$ to 0.187 at $10^{9}$, whereas the corresponding errors in $G$, $B$, $Q$ and $l_c$ change only weakly once the count is above roughly $10^6$.
...
From smearing 1 to 10 pixels, the mean absolute relative errors in $B$, $Q$ and $l_c$ rise from 0.105, 0.083 and 0.037 to 0.308, 0.326 and 0.081, respectively.
...
\midrule
PDI & 0.133 & 0.484 & 0.488 & 0.494 & 0.387 \\
PDI2 & 0.274 & 0.451 & 0.552 & 0.564 & 0.554 \\
\bottomrule
\end{tabular}
\end{table}

The distribution-resolved driver analysis also reinforces this interpretation:
\paragraph{Boltzmann.} The dominant driver of PDI error is lc ($r=0.934$), whereas the dominant driver of PDI2 error is Q ($r=0.873$).
\paragraph{Gaussian.} The dominant driver of PDI error is Q ($r=0.786$), whereas the dominant driver of PDI2 error is lc ($r=0.866$).
\paragraph{Lognormal.} The dominant driver of PDI error is lc ($r=0.961$), whereas the dominant driver of PDI2 error is Q ($r=0.867$).
\paragraph{Schulz.} The dominant driver of PDI error is B ($r=0.969$), whereas the dominant driver of PDI2 error is G ($r=0.793$).
\paragraph{Triangular.} The dominant driver of PDI error is G ($r=0.596$), whereas the dominant driver of PDI2 error is lc ($r=0.686$).
\paragraph{Uniform.} The dominant driver of PDI error is G ($r=0.870$), whereas the dominant driver of PDI2 error is lc ($r=0.314$).

\section{Distribution-Specific Interpretation}
\paragraph{Gaussian.} Across the full matrix, the mean absolute error in recovered polydispersity was 0.187 for PDI and 0.147 for PDI2, so PDI2 was the more accurate estimator on average. The best combined condition occurred at flux exponent 9 and smearing 1. At that condition, the mean relative RMS reconstruction errors were 0.066 for PDI and 0.088 for PDI2, while the full-matrix sanity-pass rate was 26.6\%.
\paragraph{Lognormal.} Across the full matrix, the mean absolute error in recovered polydispersity was 0.259 for PDI and 0.141 for PDI2, so PDI2 was the more accurate estimator on average. The best combined condition occurred at flux exponent 6 and smearing 1. At that condition, the mean relative RMS reconstruction errors were 0.239 for PDI and 6.310 for PDI2, while the full-matrix sanity-pass rate was 6.6\%.
\paragraph{Schulz.} Across the full matrix, the mean absolute error in recovered polydispersity was 0.280 for PDI and 0.255 for PDI2, so PDI2 was the more accurate estimator on average. The best combined condition occurred at flux exponent 9 and smearing 6. At that condition, the mean relative RMS reconstruction errors were 0.097 for PDI and 0.647 for PDI2, while the full-matrix sanity-pass rate was 9.0\%.
\paragraph{Boltzmann.} Across the full matrix, the mean absolute error in recovered polydispersity was 0.291 for PDI and 0.100 for PDI2, so PDI2 was the more accurate estimator on average. The best combined condition occurred at flux exponent 9 and smearing 1. At that condition, the mean relative RMS reconstruction errors were 0.163 for PDI and 0.634 for PDI2, while the full-matrix sanity-pass rate was 5.1\%.
\paragraph{Triangular.} Across the full matrix, the mean absolute error in recovered polydispersity was 0.634 for PDI and 0.173 for PDI2, so PDI2 was the more accurate estimator on average. The best combined condition occurred at flux exponent 9 and smearing 5. At that condition, the mean relative RMS reconstruction errors were 0.936 for PDI and 0.168 for PDI2, while the full-matrix sanity-pass rate was 0.0\%.
\paragraph{Uniform.} Across the full matrix, the mean absolute error in recovered polydispersity was 0.550 for PDI and 0.319 for PDI2, so PDI2 was the more accurate estimator on average. The best combined condition occurred at flux exponent 5 and smearing 3. At that condition, the mean relative RMS reconstruction errors were 1.000 for PDI and 0.323 for PDI2, while the full-matrix sanity-pass rate was 0.0\%.
